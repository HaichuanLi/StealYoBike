<script lang="ts">
	...
	const navItems = [
		...
		},
		{
			href: '/account',
			label: 'Account',
			icon: 'M12 12a5 5 0 1 0-5-5a5 5 0 0 0 5 5Zm0 2c-4 0-8 2-8 5v1h16v-1c0-3-4-5-8-5Z',
			isLoggedInNecessary: true
		}
	];
	...
	function getUserRoles(u: UserInfoResponse | null): string[] {
		if (!u) return [];
		const baseRole = u.role?.toUpperCase() || "RIDER";
		const active = u.activeRole?.toUpperCase() || baseRole;
		if (u.dualRole) {
			return [baseRole, active].map(r => r.toUpperCase());
		}
		return [baseRole];
	}
	...
	function canAccess(item: any, u: UserInfoResponse | null) {
		...
		if (item.roles && item.roles.length > 0) {
			const active = u.activeRole?.toUpperCase();
			return item.roles.includes(active);
		}
		return true;
	}
