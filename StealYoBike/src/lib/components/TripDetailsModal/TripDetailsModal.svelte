	...
	<div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
		<div class="w-[680px] max-w-[95vw] rounded-xl bg-white p-5 shadow-lg">
			<div class="mb-3 flex items-center justify-between">
				...
				<button class="text-gray-600" onclick={onClose}>✕</button>
			</div>
			...
			<div class="grid grid-cols-2 gap-3 text-sm">
				...
				<div>
					...
					{details.endTime ? new Date(details.endTime).toLocaleString() : '—'}
				</div>
			...
			<div class="my-4 rounded-lg border p-3">
				...
				<div class="grid grid-cols-2 gap-2 text-sm">
					<div>Base fee:</div>
					<div class="text-right">{money(details.baseFee)}</div>
					<div>Per-minute rate:</div>
					<div class="text-right">{money(details.perMinuteFee)}</div>
					<div>E-bike surcharge:</div>
					<div class="text-right">{money(details.eBikeSurcharge)}</div>
					<div>Discount:</div>
					<div class="text-right">{money(details.discountAmount)}</div>
					<div>Tier Discount:</div>
					<div class="text-right">{money(details.tierDiscountAmount)}</div>
					{#if details.flexDollarUsed > 0}
						<div>Flex Dollar Used:</div>
						<div class="text-right">{money(details.flexDollarUsed)}</div>
					{/if}
					<div class="font-semibold">Total:</div>
					<div class="text-right font-semibold">{money(details.totalCost)}</div>
				</div>
			</div>

			<div class="my-3">
				<p class="mb-2 text-sm text-gray-700">
					User Tier: <span class="font-semibold text-emerald-700">{details.tier}</span>
				</p>
			</div>
