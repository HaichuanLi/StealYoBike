...
class DefaultTierEvaluationServiceTest {
        ...
        void evaluate_regular_whenBronzeConditionsFail() {
                ...
                when(reservationRepo.countMissedByUserSince(eq(userId), any()))
                                .thenReturn(0);
                ...
                when(reservationRepo.countSuccessfulByUserSince(eq(userId), any()))
                                .thenReturn(0);
        ...
        void evaluate_silver_whenSilverConditionsMet() {
                ...
                when(reservationRepo.countMissedByUserSince(eq(userId), any())).thenReturn(0);
                when(tripRepo.hasUnreturnedBike(userId)).thenReturn(false);

                System.out.println("[Silver] meeting Silver conditions:");
                when(reservationRepo.countSuccessfulByUserSince(eq(userId), any())).thenReturn(6);
                System.out.println("    - successfulReservation = 6 (>=5)");
                when(tripRepo.countTripsPerMonth(eq(userId), any(), any())).thenReturn(15);
                System.out.println("    - tripsPerMonth = 15 (>=15)");

                System.out.println("[Gold] failing gold conditions");
                when(tripRepo.countTripsPerWeek(eq(userId), any(), any())).thenReturn(50);
        ...
        void evaluate_gold_whenAllConditionsMet() {
                ...
                when(reservationRepo.countSuccessfulByUserSince(eq(userId), any())).thenReturn(7);
                when(tripRepo.countTripsPerMonth(eq(userId), any(), any())).thenReturn(20);

                System.out.println("[Gold] OK");
                when(tripRepo.countTripsPerWeek(eq(userId), any(), any())).thenReturn(60);
        ...
        void downgrade_goldToSilver_whenGoldConditionFails() {
                ...
                System.out.println("    tripsPerWeek = 50 (<60 required for GOLD)");
        ...
        void downgrade_silverToBronze_whenSilverConditionsFail() {
                ...
                // Silver FAIL
                when(reservationRepo.countSuccessfulByUserSince(eq(userId), any())).thenReturn(3); // <5
        ...
        void downgrade_bronzeToRegular_whenBronzeConditionsFail() {
                ...
                // Silver irrelevant
                when(reservationRepo.countSuccessfulByUserSince(eq(userId), any())).thenReturn(10);
